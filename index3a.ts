...
function calculateOutputAmount(item : object) { //any ข้อมูลอะไรก็ได้
    ...
    let price : number  = (+item[0]); //ใส่+หน้าตัวแปร itemเพื่อ เปลี่ยน string(ข้อความ)เป็นตัวเลข
    let volume : number = (+item[1]);
    ...
    if(usdtAmount >= price*volume){ // usdtAmount(ค่าที่รับ(เงินที่มี)) มากกว่า price(ราคาเหรียญ) ที่คูณ volume เพราะราคาเหรียญเป็นราคาต่อ1เหรียญ
       ...
       let token1 : number = (usdtAmount*1)/price; //ถ้ามีเงินน้อยกว่าราคาเหรียญก็เทียบบัญญัติเพื่อซื้อ แล้วเก็บค่าใน token1           
                token = token1 + token; // เอาเหรียญที่ต้องการบวกเพิ่มไปในกระเป๋า
                usdtAmount = usdtAmount - usdtAmount; //ซื้อจนเงินหมด
             }
